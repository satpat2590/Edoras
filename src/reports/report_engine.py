...
Reports are written to ~/edoras/reports/<category>/
        ...
        for row_idx, row in enumerate(rows):
            ...
            for i, cell_data in enumerate(row):
                ...
                self.cell(
                    col_widths[i],
                    5.5,
                    self._fit_text(text, col_widths[i]),
                    border=0,
                    fill=True,
                    align=col_aligns[i],
                    new_x="END",
...
def report_positions():
    ...
    c.execute(
        "SELECT id, name, mode, initial_capital FROM portfolios WHERE is_active = 1"
    )
    ...
    for pf in portfolios:
        ...
        c.execute(
            f"""SELECT p.symbol, p.quantity, p.entry_price, p.current_price,
                            p.pnl, p.pnl_percent, p.entry_time
                     FROM positions p WHERE {filt} AND p.status = 'open'
                     ORDER BY ABS(p.quantity * p.current_price) DESC""",
            params,
        )
        ...
        c.execute(
            f"SELECT t.cash_after FROM trades t WHERE {filt_t} ORDER BY t.id DESC LIMIT 1",
            params_t,
        )
        row = c.fetchone()
...
def report_portfolio():
    ...
    c.execute(
        f"""SELECT p.symbol, p.quantity, p.current_price
    ...
    if positions:
        ...
        for p in sorted(
            positions,
            key=lambda x: x["quantity"] * (x["current_price"] or 0),
            reverse=True,
        ):
...
def report_trades():
    ...
    c.execute(
        """SELECT id, symbol, side, quantity, price, amount_usd, fee,
                        decision_context, created_at
                 FROM trades WHERE created_at > ? ORDER BY created_at""",
        (cutoff,),
    )
    trades = c.fetchall()
    ...
    c.execute(
        """SELECT symbol, entry_date, exit_date, entry_price, exit_price,
                 ...
                 FROM trade_outcomes WHERE exit_date > ? ORDER BY exit_date""",
        (cutoff,),
    )
    outcomes = c.fetchall()
    ...
    if outcomes:
        ...
        pdf.stat_cards(
            [
                (
                    "Win Rate",
                    f"{wins}/{len(outcomes)} ({100 * wins / len(outcomes):.0f}%)",
...
def report_signals():
    ...
    c.execute(
        """SELECT strategy_name, symbol, action, strength, reason,
...
def report_market():
    ...
    c.execute(
        """SELECT symbol, score, confidence, summary
    ...
    if regime:
        ...
        regime_color = (
            C_GREEN
            if "low" in regime_name.lower()
            else C_RED
            if "high" in regime_name.lower()
            else C_YELLOW
        )
    ...
    if levels:
        ...
        for lv in levels:
            ...
            trend_color = (
                C_GREEN if trend == "Above" else C_RED if trend == "Below" else C_MUTED
            )
            rows.append(
                [
                    lv["symbol"],
...
def report_risk():
    ...
    risk_state = {}
    risk_file = Path(__file__).parent / "state/risk_state.json"
    if risk_file.exists():
        try:
    ...
    max_pos = max(
        (
            p["quantity"] * (p["current_price"] or p["entry_price"]) / total_value * 100
            for p in positions
        ),
        default=0,
    )
    ...
    if cash_pct < 5:
        flags.append(
            ("LOW CASH", f"Cash at {cash_pct:.1f}% — below 5% threshold", C_RED)
        )
    if max_pos > 25:
        flags.append(
            (
                "CONCENTRATION",
                f"Largest position at {max_pos:.1f}% — exceeds 25%",
                C_RED,
            )
        )
    if cb_active:
...
def report_performance():
    ...
    c.execute(
        """SELECT symbol, outcome_pct, outcome_usd, holding_hours,
    ...
    c.execute(
        """SELECT outcome_pct, outcome_usd, signal_type
                 FROM trade_outcomes WHERE exit_date > ?""",
        (cutoff_30d,),
    )
    ...
    # Phase 3: paper_snapshots still uses portfolio_id (not in trades/positions scope)
    c.execute(
        """SELECT date, portfolio_value FROM paper_snapshots
                 WHERE portfolio_id = 1 AND date >= ? ORDER BY date""",
        (cutoff_7d[:10],),
    )
    ...
    avg_hold_7d = (
        sum(o["holding_hours"] or 0 for o in outcomes_7d) / total_7d if total_7d else 0
    )
        ...
        for s in by_signal:
            ...
            rows.append(
                [
                    ...
                    wr,
                    {
                        "text": _fmt_pct(s["avg_pct"]),
                        "color": pdf.pnl_color(s["avg_pct"] or 0),
                    },
                    {
                        "text": f"${s['total_usd']:+.2f}",
                        "color": pdf.pnl_color(s["total_usd"] or 0),
                    },
                ]
            )
        pdf.table(headers, rows, widths, aligns)
