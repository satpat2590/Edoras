...
class PriceAlertSystem:
    ...
    def __init__(
        self, api_key: str, api_secret: str, alert_file: str = "price_alerts.json"
    ):
    ...
    def check_alerts(self, prices: Dict[str, Dict]) -> List[Dict]:
        ...
        for symbol, price_data in prices.items():
            ...
            thresholds = self.alerts.get(
                symbol, self.default_thresholds.get(symbol, {})
            )
            ...
            if (
                "change_24h" in thresholds
                and abs(change_24h) > thresholds["change_24h"]
            ):
    ...
    def filter_duplicate_alerts(self, alerts: List[Dict]) -> List[Dict]:
        ...
        for alert in alerts:
            ...
            for hist_alert in self.alert_history.get("alerts", []):
                if (
                    hist_alert.get("symbol") == alert["symbol"]
                    and hist_alert.get("type") == alert["type"]
                ):
                    # Parse timestamp
                    try:
                        alert_time = datetime.fromisoformat(
                            hist_alert["timestamp"].replace("Z", "+00:00")
                        ).timestamp()
    ...
    def send_alert_via_telegram(self, message: str, target: str = None) -> bool:
        """Send alert via Telegram Bot API"""
        target = target or os.getenv("TELEGRAM_CHAT_ID", "1806720995")
        try:
            ...
            data = urllib.parse.urlencode({"chat_id": target, "text": message}).encode()
    ...
    def run_check(
        self, symbols: Optional[List[str]] = None, target: str = None
    ) -> bool:
        ...
        success = self.send_alert_via_telegram(alert_message, target)
    ...
    try:
        ...
        if args.configure:
            ...
            alert_system.configure_alert(
                symbol=args.symbol, high=args.high, low=args.low, change_24h=args.change
            )
            ...
            if choice == "1":
                symbols_input = input(
                    "Symbols to check (comma-separated, blank for default): "
                ).strip()
                symbols = (
                    [s.strip() for s in symbols_input.split(",")]
                    if symbols_input
                    else None
                )
                ...
                if history.get("alerts"):
                    ...
                    for alert in history["alerts"][-5:]:  # Last 5 alerts
                        print(
                            f"  {alert.get('timestamp', 'Unknown')}: {alert.get('message', 'No message')}"
                        )
                else:
