...
LIVENESS_INTERVAL = 300  # liveness probe every 5 min
SYMBOL_IDLE_WARN = 300  # warn if a symbol has no ticks for 5 min
FEED_IDLE_CRITICAL = 600  # force reconnect if NO symbols send ticks for 10 min
RECONNECT_BASE = 1
RECONNECT_CAP = 60
...
class BaseWebSocketClient(abc.ABC):
    ...
    def __init__(self, symbols: List[str], db_path: str, symbol_token_map: Dict[str, str] = None):
        ...
        self._zombie_reconnect_requested = False  # set True to break the message loop
    ...
    async def _flush_candles(self):
        ...
        keys_to_flush = [
            k
            for k in list(self._candles_5m.keys())
    ...
    def _recompute_indicators(self, symbols: List[str], timeframes: tuple = ("1h",)):
        ...
        Delta-based: only reads recent candles needed for indicator warmup
        (last 250 candles, enough for SMA-200), not entire history.
        """
        import sys
        import os
            ...
            for symbol in symbols:
                ...
                for tf in timeframes:
                    ...
                    for _, irow in df.iterrows():
                        ...
                        rows.append((symbol, tf, int(irow["timestamp"]), *vals))
                    ...
                    if rows:
                        ...
                        conn.commit()
                    logger.info(
                        f"[{self.exchange_code}] Indicators recomputed: {symbol}/{tf} (profile={profile})"
                    )
            conn.close()
        except Exception as e:
    ...
    async def _message_loop(self, ws):
        ...
        self._zombie_reconnect_requested = False
        async for raw in ws:
            try:
                ...
                if now - self._last_stats >= STATS_INTERVAL:
                    self._log_stats()
                # Liveness probe — break the loop to force reconnect if zombie detected
                if now - self._last_liveness_check >= LIVENESS_INTERVAL:
                    if not self._check_liveness():
                        break
                # Dynamic subscription refresh
                if (
                    self.sub_refresh_interval > 0
                    and now - self._last_sub_refresh >= self.sub_refresh_interval
                ):
        ...
        # If we exited due to zombie detection, raise to trigger reconnect logic
        if self._zombie_reconnect_requested:
            raise ConnectionError(f"[{self.exchange_code}] Zombie feed — forced reconnect")
    ...
    def _check_liveness(self) -> bool:
        """Check for zombie connections (connected but no ticks flowing).

        Warns when individual symbols go idle for SYMBOL_IDLE_WARN seconds.
        Requests a forced reconnect when the entire feed produces no ticks for
        FEED_IDLE_CRITICAL seconds.

        Returns True if everything looks alive, False if a reconnect is needed.
        """
        now = time.monotonic()
        self._last_liveness_check = now

        if not self._last_tick_time:
            # No ticks seen yet — could still be starting up; don't flag immediately
            return True

        # Per-symbol idle warnings
        idle_symbols = []
        for sym in self.symbols:
            last = self._last_tick_time.get(sym)
            if last is None:
                continue  # symbol may legitimately have no data (e.g. low-volume market)
            idle_secs = now - last
            if idle_secs > SYMBOL_IDLE_WARN:
                idle_symbols.append((sym, idle_secs))

        if idle_symbols:
            idle_list = ", ".join(
                f"{s}({idle:.0f}s)" for s, idle in sorted(idle_symbols, key=lambda x: -x[1])[:5]
            )
            logger.warning(
                f"[{self.exchange_code}] Liveness: {len(idle_symbols)} idle symbol(s) — {idle_list}"
            )

        # Feed-level critical: check when the most recent tick from ANY symbol was
        most_recent = max(self._last_tick_time.values(), default=None)
        if most_recent is None:
            return True

        feed_idle_secs = now - most_recent
        if feed_idle_secs > FEED_IDLE_CRITICAL:
            logger.error(
                f"[{self.exchange_code}] ZOMBIE FEED DETECTED: no ticks from any symbol "
                f"for {feed_idle_secs:.0f}s (threshold {FEED_IDLE_CRITICAL}s) — forcing reconnect"
            )
            self._zombie_reconnect_requested = True
            return False
