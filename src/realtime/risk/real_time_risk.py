...
class RealTimeRiskManager:
    ...
    def _send_telegram(self, message: str, critical: bool = False):
        """Send a Telegram notification for risk events."""
        bot_token = os.getenv("TELEGRAM_BOT_TOKEN", "")
        chat_id = os.getenv("TELEGRAM_CHAT_ID", "")
        if not bot_token or not chat_id:
            logger.warning("Telegram env vars not set — real-time risk alert not sent")
            return
        if len(message) > 3900:
            message = message[:3900] + "\n..."
        prefix = "🚨 CRITICAL" if critical else "⚠️ RISK"
        full_msg = f"{prefix} — {datetime.now().strftime('%H:%M:%S')}\n{message}"
        try:
            subprocess.run(
                [
                    "curl",
            ...
            logger.info(f"Real-time risk alert sent (critical={critical})")
        except Exception as e:
            logger.warning(f"Real-time risk Telegram alert failed: {e}")

    async def connect_to_db(self):
        """Connect to database"""
        self.db_pool = await aiosqlite.connect(self.db_path)
        # Phase 3: resolve account IDs for the default portfolio
        self._account_ids = config.get_account_ids(
            config.DEFAULT_PORTFOLIO_ID, db_path=self.db_path
        )
        logger.info(
            f"Risk manager connected to database (accounts: {self._account_ids})"
        )
            ...
            for position in positions:
                ...
                if take_profit_levels_json:
                    try:
                        ...
                        take_profit_levels = {
                            str(level): False for level, _ in self.take_profit_levels
                        }
                ...
                for level, sell_pct in self.take_profit_levels:
                    level_str = str(level)
                    if gain_pct >= level and not take_profit_levels.get(
                        level_str, False
                    ):
                        # Partial exit
    ...
    async def trigger_exit(
        ...
        logger.info(
            f"🚨 {exit_type}: {symbol} {quantity:.6f} @ ${price:.2f} - {reason}"
        )
    ...
    async def trigger_partial_exit(
        self,
        position_id: int,
        symbol: str,
        quantity: float,
        ...
        logger.info(
            f"🎯 Partial {exit_type}: {symbol} {quantity:.6f} ({sell_pct * 100:.0f}%) @ ${price:.2f}"
        )
        ...
        try:
            ...
            self._send_telegram(
                ...
                f"Sold {sell_pct * 100:.0f}%  ({quantity:.6f})  @ ${price:.4f}\n"
                f"Reason: {reason[:200]}"
            )
    ...
    async def check_circuit_breaker(self):
        ...
        try:
            ...
            peak_value = (
                peak_result[0] if peak_result and peak_result[0] else current_value
            )
            ...
            if drawdown >= self.circuit_breaker_pct:
                logger.warning(
                    f"⚠️ Circuit breaker threshold reached: {drawdown * 100:.1f}% drawdown"
                )
                ...
                self._send_telegram(
                    ...
                    f"All {len(positions)} position(s) liquidated.",
                    critical=True,
                )
    ...
    async def update_portfolio_snapshot(self):
        ...
        try:
            ...
            if self._account_ids:
                ...
                count_row = await (
                    await self.db_pool.execute(count_sql, self._account_ids)
                ).fetchone()
            ...
            logger.debug(
                f"Portfolio snapshot: total=${total_value:.2f}, cash=${cash:.2f}, invested=${invested:.2f}"
            )
