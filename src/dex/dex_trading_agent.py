...
def update_dex_data():
    ...
    from data.dex_data_collector import DexDataCollector
...
def compute_dex_indicators():
    ...
    from data.compute_all_indicators import compute_all_indicators
...
def gather_dex_context() -> dict:
    ...
    # Per-symbol context
    from dex.dex_risk_rules import DexRiskRules
    risk = DexRiskRules()
    ...
    # Exit overlay signals for held positions
    ctx["exit_overlay_signals"] = []
    if ctx.get("positions"):
        try:
            from core.exit_overlay import ExitOverlay
            eo = ExitOverlay(db_path=DB_PATH)
            # Build positions dict in the format exit_overlay expects
            pos_dict = {
                sym: {"quantity": p["quantity"], "avg_price": p["entry_price"]}
                for sym, p in ctx["positions"].items()
                if p.get("quantity", 0) > 0
            }
            if pos_dict:
                exit_sigs = eo.check_all_exits(pos_dict, DB_PATH)
                ctx["exit_overlay_signals"] = exit_sigs
                if exit_sigs:
                    logger.info(f"Exit overlay: {len(exit_sigs)} signals for DEX positions")
        except ImportError:
            logger.debug("Exit overlay not available for DEX context")
        except Exception as e:
            logger.warning(f"Exit overlay failed for DEX: {e}")
...
def _format_exit_signals(ctx: dict) -> str:
    """Format exit overlay signals for the LLM prompt."""
    signals = ctx.get("exit_overlay_signals", [])
    if not signals:
        return "  No exit signals — all held positions pass exit checks."
    lines = []
    for sig in signals:
        lines.append(
            f"  {sig['symbol']}: SELL (strength {sig['strength']:.0f}) — {sig.get('reason', sig.get('exit_type', ''))}"
        )
    return "\n".join(lines)
...
def build_dex_prompt(ctx: dict) -> str:
    ...
    prompt = f"""You are Argus, managing the Arwen DEX portfolio on Base chain via Bankr.
...
## Exit Overlay Signals (automated exit analysis for held positions)
{_format_exit_signals(ctx)}
...
def call_llm(prompt: str) -> dict:
    """Call LLM for trading decision. Tries OpenAI, then MLX fallback."""
    import openai
    import re

    messages = [
        {"role": "system", "content": "You are a quantitative DEX trading agent. Always respond with valid JSON."},
    ...
    try:
        ...
        logger.info(f"OpenAI responded ({len(text)} chars)")
    except Exception as e:
        logger.warning(f"OpenAI failed: {e}")
        text = None

    # Fallback to Mac Mini MLX
    if text is None:
        try:
            ...
            text = response.choices[0].message.content.strip()
            logger.info(f"MLX (Mac Mini) responded ({len(text)} chars)")
        except Exception as e:
            logger.warning(f"MLX (Mac Mini) failed: {e}")
            return {"assessment": f"All LLM providers failed", "trades": []}
