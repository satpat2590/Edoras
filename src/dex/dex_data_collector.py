...
class DexDataCollector:
    ...
    def collect_all(self, backfill_days: int = 1):
        ...
        # Uses the standalone compute_all_indicators which needs no API credentials
        # (the old CryptoDataCollector path required Coinbase creds just to init).
        try:
            from data.compute_all_indicators import compute_all_indicators
            compute_all_indicators(db_path=self.db_path)
