...
def retry_with_exponential_backoff(
    ...
    skip_on_messages: tuple = ("INVALID_ARGUMENT", "ProductID is invalid"),
):
    ...
    Args:
        ...
        skip_on_messages: Tuple of error message substrings that should skip retry
    """
        ...
        def wrapper(*args, **kwargs):
            ...
            for attempt in range(max_retries + 1):
                try:
                    ...
                    # Skip retry if error message contains skip substrings
                    if any(skip in str(e) for skip in skip_on_messages):
                        logger.warning(f"Error contains skip pattern, not retrying: {e}")
                        raise
...
class CryptoDataCollector:
    ...
    def init_database(self):
        ...
        # Performance indexes
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_candlesticks_sym_tf ON candlesticks(symbol, timeframe)"
        )
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_candlesticks_sym_tf_ts ON candlesticks(symbol, timeframe, timestamp DESC)"
        )
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_indicators_sym_tf ON indicators(symbol, timeframe)"
        )
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_indicators_sym_tf_ts ON indicators(symbol, timeframe, timestamp DESC)"
        )
    ...
    def get_portfolio_symbols(self) -> List[str]:
        ...
        logger.info(
            f"Using {len(supported_symbols)} hardcoded portfolio symbols: {supported_symbols}"
        )
    ...
    def save_candlesticks(self, symbol: str, timeframe: str, candles: List[Dict]):
        """Save candlestick data to database using batch insert"""
        if not candles:
            return 0
    ...
    def aggregate_4h_candles(self, symbol: str, lookback_days: int = 14) -> int:
        ...
        df["bucket"] = (df["timestamp"] // 14400) * 14400
    ...
    def calculate_indicators(self, symbol: str, timeframe: str, incremental: bool = True):
        """Calculate technical indicators for symbol/timeframe.

        When incremental=True (default), only calculates for new candles
        plus the warmup period needed for indicator stability.
        """
        ...
        if incremental:
            ...
            if last_ts > 0:
                ...
                """
                df = pd.read_sql_query(query, conn, params=(symbol, timeframe, last_ts - 3600))
            else:
                query = """
                SELECT timestamp, open, high, low, close, volume
                FROM candlesticks
    ...
    def generate_portfolio_report(self) -> str:
        ...
        for symbol in symbols:
            ...
            if not daily_df.empty and daily_df.iloc[0]["action"]:
                ...
                report_lines.append(
                    f"  💡 **Recommendation**: {action} ({confidence:.0f}% confidence)"
                )
                if reasoning:
        ...
        report_lines.append(
            "• Short-term (1h): Look for RSI extremes (<30 oversold, >70 overbought)"
        )
        report_lines.append(
            "• Medium-term (4h): MACD histogram crossing zero indicates momentum shift"
        )
    ...
    def run_daily_collection(self):
        ...
        logger.info(
            f"Processing {len(all_symbols)} symbols ({len(portfolio_symbols)} portfolio, {len(all_symbols) - len(portfolio_symbols)} top cryptos)"
        )

        # Update data for all symbols and timeframes.
        # NOTE: '4h' is NOT fetched from REST (Coinbase doesn't serve 4h directly).
        # 4h candles are built by aggregate_4h_candles() below from 1h data.
        for symbol in all_symbols:
            ...
            # Aggregate 4h from updated 1h data
            try:
                self.aggregate_4h_candles(symbol, lookback_days=7)
                self.calculate_indicators(symbol, "4h")
            except Exception as e:
                logger.error(f"Error aggregating 4h for {symbol}: {e}")
...
def main():
    ...
    try:
        collector = CryptoDataCollector(
            db_path=_CONFIG_DB_PATH, api_key=api_key, api_secret=api_secret
        )
        ...
        if args.init:
            ...
            with open("reports/portfolio_technical_report.txt", "w") as f:
                f.write(report)
            print("Report saved to reports/portfolio_technical_report.txt")
