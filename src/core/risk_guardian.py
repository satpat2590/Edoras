...
RISK_EVENTS_FILE = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "risk_events.jsonl"
)
...
class RiskGuardian:
    ...
    def run_check(self) -> Dict:
        ...
        # Run all risk checks
        exit_signals, cb, violations = self.risk_manager.run_all_checks(
        ...
        if exits > 0 or viols > 0:
            logger.info(
                f"Risk guardian: {exits} exits, {viols} violations, CB={result['circuit_breaker']}"
            )
        else:
            logger.info(
                f"Risk guardian: all clear ({result['positions_checked']} positions checked)"
            )
    ...
    def _sync_positions(self):
        ...
        for sym, pos in self.portfolio.positions.items():
            if sym not in self.risk_manager.entry_prices:
                ...
                logger.info(
                    f"Synced position {sym} to risk manager (entry ${pos['avg_price']:.4f})"
                )
    ...
    def _execute_exit(self, sig: ExitSignal) -> bool:
        ...
        self.portfolio.set_trade_context(
            sym, signal_type="risk_exit", exit_reason=sig.exit_type
        )
        ...
        if sig.quantity_pct >= 1.0:
            ...
            if success:
                logger.info(
                    f"EXECUTED: Partial exit {sym} {sig.quantity_pct:.0%} ({sig.exit_type})"
                )
                return True
    ...
    def _store_in_market_intel(self, result: Dict):
        ...
        try:
            ...
            for exit in result["exits_triggered"]:
                parts.append(
                    f"Risk exit: {exit['type']} on {exit['symbol']} at ${exit.get('price', 0):.2f}. {exit['reason']}"
                )
            if result["circuit_breaker"]:
                parts.append(
                    f"Circuit breaker activated. {result.get('circuit_breaker_detail', '')}"
                )
    ...
    def _send_alert(self, result: Dict):
        ...
        if result["circuit_breaker"]:
            lines.append(
                f"🚨 {result.get('circuit_breaker_detail', 'CIRCUIT BREAKER ACTIVATED')}"
            )
            lines.append("")

        for exit in result["exits_triggered"]:
            emoji = {"stop_loss": "🛑", "trailing_stop": "⚡", "take_profit": "💰"}.get(
                exit["type"], "⚠️"
            )
        ...
        try:
            cmd = [
                ...
                "-X",
                "POST",
                f"https://api.telegram.org/bot{os.getenv('TELEGRAM_BOT_TOKEN', '8724014451:AAGpisAWj86i8qmkOtfb4mCBSpiPfZd0ROI')}/sendMessage",
                "-d",
                f"chat_id={os.getenv('TELEGRAM_CHAT_ID', '1806720995')}",
                "-d",
                f"text={message}",
            ]
...
def main():
    ...
    parser.add_argument(
        "--hours", type=int, default=24, help="Hours lookback for recent events"
    )
    ...
    if args.recent:
        ...
        if events:
            for e in events:
                print(
                    f"[{e['timestamp']}] Exits: {len(e.get('exits', []))} CB: {e.get('circuit_breaker', False)}"
                )
