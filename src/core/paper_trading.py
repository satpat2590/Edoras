...
PORTFOLIO_STATE_FILE = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "state/paper_portfolio_full_state.json"
)
...
class PaperTradingPortfolio:
    ...
    def __init__(
        ...
        portfolio_id: int = ACTIVE_PORTFOLIO_ID,
    ):
        ...
        if not self._load_state():
            ...
            logger.info(
                f"Paper trading portfolio loaded: ${self.capital:.2f} cash, {len(self.positions)} positions"
            )
    ...
    def _save_trade_to_db(self, trade: dict):
        ...
        try:
            ...
            if (
                ctx is None
                and hasattr(self, "_pending_decision_context")
                and self._pending_decision_context
            ):
    ...
    def execute_buy(self, symbol: str, amount_usd: float, timestamp: datetime = None):
        ...
        if not self._has_decision_context(symbol):
            logger.warning(
                f"REJECTED buy {symbol}: no decision_context — set context before trading"
            )
            return False
        ...
        # Auto-enroll new symbol into signal engine if this is the first buy
        if is_new_position:
            try:
                from backtest.deployer import enroll_symbol

                enroll_result = enroll_symbol(
                    symbol, portfolio_id=self.portfolio_id, db_path=self.db_path
                )
                if enroll_result.get("enrolled"):
                    logger.info(f"Auto-enrolled {symbol} → {enroll_result['strategy']}")
            except Exception as e:
                logger.warning(f"Failed to auto-enroll {symbol}: {e}")
        ...
        logger.info(
            f"Bought {quantity:.6f} {symbol} at ${price:.2f} for ${amount_usd:.2f} (cost: ${cost:.2f})"
        )
        return True
    ...
    def execute_sell(self, symbol: str, quantity: float, timestamp: datetime = None):
        ...
        if not self._has_decision_context(symbol):
            logger.warning(
                f"REJECTED sell {symbol}: no decision_context — set context before trading"
            )
            return False
            ...
            if journal:
                try:
                    journal.record_outcome(
                        ...
                        exit_price=price,
                        quantity=current_qty,
                        signal_type=self._last_signal_type.get(symbol)
                        if hasattr(self, "_last_signal_type")
                        else None,
                        signal_strength=self._last_signal_strength.get(symbol)
                        if hasattr(self, "_last_signal_strength")
                        else None,
                        exit_reason=self._last_exit_reason.get(symbol, "signal")
        ...
        self._save_trade_to_db(trade)
        logger.info(
            f"Sold {quantity:.6f} {symbol} at ${price:.2f} for ${proceeds:.2f} (cost: ${cost:.2f})"
        )
        return True
    ...
    def generate_performance_report(self) -> str:
        ...
        if self.positions:
            ...
            from reports.telegram_fmt import fmt_positions
        ...
        if self.trade_history:
            ...
            for trade in recent_trades:
                ...
                report_lines.append(
                    f"• {timestamp} {trade_type} {quantity:.4f} {symbol} @ ${price:.2f} (${amount:.2f})"
                )
        ...
        report_lines.append("---")
        report_lines.append(
            "_Paper Trading Simulation • Not real money • For educational purposes_"
        )
...
def main():
    ...
    parser.add_argument(
        "--init", action="store_true", help="Initialize portfolio with top 5 cryptos"
    )
    ...
    try:
        portfolio = PaperTradingPortfolio(_CONFIG_DB_PATH, initial_capital=1000.0)

        if args.init:
            # Get top 5 symbols from enhanced optimizer
            from scoring.enhanced_optimizer import EnhancedPortfolioOptimizer

            optimizer = EnhancedPortfolioOptimizer(_CONFIG_DB_PATH)
            ...
            if not scores_df.empty:
                ...
                with open("state/paper_portfolio_state.json", "w") as f:
                ...
                with open("reports/paper_portfolio_report.txt", "w") as f:
                    f.write(report)
                print("\n✅ Report saved to reports/paper_portfolio_report.txt")
            else:
