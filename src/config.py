...
# ── Paths ────────────────────────────────────────────────────────────────
# BASE_DIR is the project root (one level up from src/config.py)
_THIS_FILE = os.path.abspath(__file__)
BASE_DIR = os.path.normpath(os.path.join(os.path.dirname(_THIS_FILE), ".."))
...
PAPER_STATE_FILE = os.path.join(BASE_DIR, "state/paper_portfolio_state.json")
RISK_STATE_FILE = os.path.join(BASE_DIR, "state/risk_state.json")
SIGNAL_STATE_FILE = os.path.join(BASE_DIR, "state/signal_trading_state.json")
...
TIMEFRAMES = {
    "1h": "ONE_HOUR",
    "1d": "ONE_DAY",
}  # 4h is aggregated from 1h (see crypto_data_collector.aggregate_4h_candles)
...
CIRCUIT_BREAKER_COOLDOWN_H = 24  # hours before auto-reset eligible
CIRCUIT_BREAKER_CASH_RATIO = 0.80  # reset early if cash >= 80% of portfolio
...
def get_active_portfolios(db_path: str = None) -> list:
    ...
    for r in rows:
        d = dict(r)
        d["symbols"] = (
            json.loads(d.pop("symbols_json")) if d.get("symbols_json") else PORTFOLIO_SYMBOLS
        )
        d["strategy_routes"] = (
            json.loads(d.pop("strategy_routes_json")) if d.get("strategy_routes_json") else {}
        )
        result.append(d)
    return result
...
def resolve_account_id(
    portfolio_id: int, venue_id: int = None, venue_code: str = None, db_path: str = None
) -> int:
    ...
    conn.close()
    if not row:
        raise ValueError(f"No active account for portfolio_id={portfolio_id}, venue_id={venue_id}")
