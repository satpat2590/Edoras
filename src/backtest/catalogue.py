...
class StrategyCatalogue:
    ...
    def record(
        self, result: BacktestResult, tags: str = "", notes: str = "",
        source: str = "backtest",
    ) -> int:
        """Record a backtest result in the catalogue. Returns the row ID.

        source: 'backtest', 'walk_forward', 'holdout_gate', or 'portfolio_backtest'
        """
        m = result.metrics
        metrics_dict = {
        ...
        has_source = any(
            row[1] == "source"
            for row in self.conn.execute("PRAGMA table_info(strategy_catalogue)").fetchall()
        )
        ...
        if has_source:
            row_id = self.conn.execute("""
                INSERT INTO strategy_catalogue (
                    ...
                    metrics_json, parameters_json, tags, notes, source
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (
    ...
    def record_performance(
        ...
        """Record a strategy_performance row linked to a catalogue entry.

        If catalogue_id is None, attempts to find a matching catalogue row.
        Returns the performance row ID.
        """
        m = result.metrics
        row_id = self.conn.execute("""
            INSERT INTO strategy_performance (
                strategy_name, symbol, timeframe, source,
                period_start, period_end,
                total_return, annualized_return, sharpe_ratio, sortino_ratio,
                max_drawdown, win_rate, profit_factor, total_trades,
                avg_win, avg_loss, avg_holding_days, calmar_ratio,
                parameters_json, catalogue_id
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (
            result.strategy_name, result.symbol, result.timeframe, source,
            result.start_date, result.end_date,
            m.total_return, m.annualized_return, m.sharpe_ratio, m.sortino_ratio,
            m.max_drawdown, m.win_rate, m.profit_factor, m.total_trades,
            m.avg_win, m.avg_loss, m.avg_holding_days, m.calmar_ratio,
            json.dumps(result.parameters), catalogue_id,
        )).lastrowid
        self.conn.commit()
        return row_id
