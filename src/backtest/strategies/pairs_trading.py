...
class PairsTradingStrategy(Strategy):
    ...
    def __init__(
        ...
        pair_symbol: str = "ETH-USD",  # reference pair symbol
    ):
        ...
        self.required_symbols = [pair_symbol]
    ...
    def generate_signals_multi(
        self, data: dict, portfolio: dict
    ) -> list:
        """Multi-symbol pairs trading using actual spread between two assets.

        data: {primary_symbol: df, pair_symbol: df}
        Computes the log-price ratio spread and trades z-score mean-reversion.
        Falls back to self-referential spread if pair data is missing.
        """
        symbols = list(data.keys())
        primary_sym = portfolio.get("symbol", symbols[0])
        primary_df = data.get(primary_sym)

        # Find the pair DataFrame
        pair_df = None
        ...
        if pair_df is None or len(pair_df) < self.lookback + 10:
            # Fall back to self-referential spread
            return self.generate_signals(primary_df, portfolio)

        if len(primary_df) < self.lookback + 10:
            return []

        # Align on timestamps
        p1 = primary_df.set_index("timestamp")["close"]
        p2 = pair_df.set_index("timestamp")["close"]
        aligned = pd.DataFrame({"p1": p1, "p2": p2}).dropna()

        if len(aligned) < self.lookback + 10:
            return self.generate_signals(primary_df, portfolio)

        # Log-price ratio spread
        spread = np.log(aligned["p1"]) - np.log(aligned["p2"])
        z_series = _compute_spread_zscore(spread, self.lookback)
        ...
        hl = _half_life(spread.iloc[-self.lookback:])
        if hl > self.max_half_life:
            return []

        bar = primary_df.iloc[-1]
        ...
        if z < -self.entry_z and portfolio["position_qty"] == 0:
            signals.append({
                ...
                "reason": (
                    f"Pairs BUY (real spread): z={z:.2f} "
            ...
            if z > self.entry_z:
                signals.append({
                    "action": "SELL",
                    "weight": 1.0,
                    "reason": f"Pairs SELL (overshoot): z={z:.2f} half_life={hl:.1f}d",
                })
            elif abs(z) < self.exit_z:
                signals.append({
                    "action": "SELL",
                    "weight": 1.0,
                    "reason": f"Pairs SELL (mean-revert): z={z:.2f} half_life={hl:.1f}d",
                })

        return signals

    def generate_signals(self, df: pd.DataFrame, portfolio: dict) -> list:
        """Single-symbol fallback: self-referential spread z-score."""
