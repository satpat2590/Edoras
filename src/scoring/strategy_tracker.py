...
try:
    from config import ACTIVE_PORTFOLIO_ID, DB_PATH
except ImportError:
...
class StrategyTracker:
    ...
    def update_signal_outcome(
        ...
        conn = self._connect()
        conn.execute(
            "UPDATE strategy_signals_log SET outcome_pct=?, exit_price=?, exit_reason=? WHERE id=?",
    ...
    def get_strategy_detail(self, strategy_name: str, source: str = None) -> List[dict]:
        ...
        if source:
            ...
            params.append(source)
        rows = conn.execute(
            f"SELECT * FROM strategy_performance {where} ORDER BY sharpe_ratio DESC",
            params,
        ).fetchall()
        conn.close()
        return [dict(r) for r in rows]
...
def format_strategy_report(tracker: StrategyTracker) -> str:
    ...
    if summary:
        ...
        lines.append(
            f"{'Strategy':<22} {'TF':>4} {'Source':>9} {'Tests':>5} {'AvgRet':>8} {'AvgSharpe':>9} {'AvgWR':>6} {'Pass':>5}"
        )
        lines.append("-" * 75)
        for s in summary:
            lines.append(
    ...
    if signals:
        ...
        lines.append(
            f"{'Strategy':<22} {'Symbol':<12} {'Action':>6} {'Signals':>8} {'Exec':>5} {'AvgOut':>8} {'W/L':>6}"
        )
        lines.append("-" * 75)
        for s in signals:
