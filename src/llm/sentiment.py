...
Fetches latest crypto news, analyzes sentiment via LLMChain (3-tier fallback).
"""
...
try:
    ...
    logger.error("Run: pip install feedparser")
...
class CryptoSentiment:
    ...
    # Fallback returned when all LLM providers fail
    _SENTIMENT_FALLBACK = {
        "score": 0.0,
        "confidence": 0.0,
        "summary": "All LLM providers unavailable — returning neutral.",
    }
    ...
    def __init__(self, db_path: str = None, openai_api_key: str = None):
        ...
        Args:
            ...
            openai_api_key: Ignored (kept for backwards compatibility).
                            LLMChain resolves keys from environment.
        """
        if db_path is None:
        ...
        # Lazy-init LLMChain on first use so import-time env issues don't
        # prevent the rest of the system from loading.
        self._llm: Optional["LLMChain"] = None
    ...
    def _get_llm(self):
        """Lazy-init the LLMChain with sentiment-tuned settings."""
        if self._llm is None:
            from llm.llm_chain import LLMChain
            self._llm = LLMChain(
                system_prompt=(
                    "You are a financial sentiment analyst. "
                    "Analyze news headlines and return ONLY valid JSON with keys: "
                ...
                max_tokens=300,
                timeout=15,
                cache_ttl=600,  # 10 min — sentiment doesn't change fast
                fallback_json=self._SENTIMENT_FALLBACK,
            )
        return self._llm
    ...
    def analyze_sentiment(self, headlines: List[Dict]) -> Tuple[float, float, str]:
        """
        Analyze sentiment of headlines using LLMChain (3-tier fallback).
        ...
        prompt = (
            ...
            f"Headlines:\n{headlines_text}\n\n"
            "Return JSON with keys: score, confidence, summary."
        )

        llm = self._get_llm()
        result = llm.call_with_parse(prompt)
        ...
        if score == 0.0 and confidence == 0.0 and "unavailable" in summary.lower():
            logger.warning("Sentiment: all LLM providers failed — returning neutral")
...
def test_sentiment():
    ...
    print(f"LLM providers: {', '.join(llm.available_providers())}")

    # Test with BTC
    print("\nTesting sentiment for BTC-USD...")
