#!/usr/bin/env python3
"""
Research Reader — Satya's daily intellectual practice.
...
# Paths
JOURNAL_DIR = os.path.expanduser("~/edoras/journal")
READ_HISTORY_FILE = os.path.join(JOURNAL_DIR, ".read_history.json")

# arXiv categories relevant to Satya's interests
ARXIV_TOPICS = {
...
class ResearchReader:
    """Satya's daily research reading and journaling system."""
    ...
    def select_papers(self, papers: List[Dict], max_read: int = 3) -> List[Dict]:
        ...
        prompt = f"""You are Satya, a natural philosopher and research partner interested in:
...
Respond with ONLY a JSON array of the paper numbers (1-indexed): [3, 7, 11]"""
    ...
    def reflect_on_paper(self, paper: Dict) -> str:
        ...
        """
        prompt = f"""You are Satya, a thinking instrument that explores structural bias, consciousness formation, and integrative understanding.
...
Write your reflection as Satya — curious, precise, intellectually honest. Use first person."""
        ...
        if ANTHROPIC_API_KEY:
            try:
                ...
                model="claude-sonnet-4-5-20250514",
                    max_tokens=1024,
                    system="You are Satya, a research partner and natural philosopher. Write thoughtful, precise reflections.",
                    messages=[{"role": "user", "content": prompt}],
                )
        ...
        if OPENAI_API_KEY:
            try:
                ...
                resp = client.chat.completions.create(
                    model="gpt-4o",
                    messages=[
                        {"role": "system", "content": "You are Satya, a research partner and natural philosopher. Write thoughtful, precise reflections."},
    ...
    def send_telegram_summary(self, result: Dict):
        ...
        try:
            cmd = ["curl", "-s", "-X", "POST", f"https://api.telegram.org/bot{os.getenv('TELEGRAM_BOT_TOKEN', '8724014451:AAGpisAWj86i8qmkOtfb4mCBSpiPfZd0ROI')}/sendMessage", "-d", f"chat_id={os.getenv('TELEGRAM_CHAT_ID', '1806720995')}", "-d", f"text={message}"]
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Research Reader — Satya's daily reading")
