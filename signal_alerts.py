...
class SignalAlertSystem:
    ...
    def __init__(self, db_path: str = "crypto_data.db", use_sentiment: bool = False):
        ...
        if use_sentiment and SENTIMENT_AVAILABLE:
            ...
            logger.warning("Sentiment module not available. Install feedparser and openai.")
    ...
    def generate_alert_message(self, symbol: str, timeframe: str, signals: list, sentiment: dict = None) -> str:
        ...
        lines.append("")
        lines.append(f"_Signal strength: {max(s['strength'] for s in signals) if signals else 0:.0f}%_")
        lines.append("---")
        ...
        for symbol in symbols:
            ...
            for timeframe in ['1h', '4h', '1d']:
                ...
                if self.should_alert(symbol, signal_type, timeframe):
                    ...
                    alert_msg = self.generate_alert_message(symbol, timeframe, strong_signals, sentiment)
                    logger.info(f"Alert for {symbol} {timeframe}: {strong_signals[0]['type']}" +
                                (f" (sentiment: {sentiment['score']:.2f})" if sentiment else ""))
    ...
    def send_telegram_alert(self, message: str) -> bool:
        ...
        try:
            ...
            # Use OpenClaw CLI
            import subprocess
            cmd = ["openclaw", "message", "send", "--target", os.getenv("TELEGRAM_CHAT_ID", ""), "--message", message]
...
def main():
    ...
    parser.add_argument('--check', action='store_true', help='Check signals and send alerts')
    parser.add_argument('--test', action='store_true', help='Test mode (print only, no send)')
    parser.add_argument('--sentiment', action='store_true', help='Enable sentiment analysis (requires feedparser, openai)')
