#!/usr/bin/env python3
"""
Trading Agent — AI-powered strategic trading decision engine.

Collects all available market data, risk state, and historical context,
then uses an LLM to reason about trade decisions. Executes via paper trading.
Stores analysis in market intelligence for future reference.

This is Aleph's trading brain.
"""
...
from config import (
    DB_PATH, PAPER_STATE_FILE, TELEGRAM_CHAT_ID,
    PORTFOLIO_SYMBOLS, TOP_CRYPTO_SYMBOLS, EQUITY_SYMBOLS,
    STOP_LOSS_PCT, TRAILING_STOP_ACTIVATION, TAKE_PROFIT_LEVELS,
    MAX_PORTFOLIO_DRAWDOWN, MAX_POSITION_PCT, MAX_SECTOR_PCT,
    get_asset_type, get_sector,
)
...
class TradingAgent:
    """
    Strategic trading agent that combines quantitative signals with
    ...
    def optimizer(self):
        if self._optimizer is None:
            ...
            self._optimizer = EnhancedPortfolioOptimizer(db_path=self.db_path)
    ...
    def classify_trend(self, symbol: str, timeframe: str = "1h") -> Dict:
        ...
        if not row:
            return {"trend": "unknown", "strength": "unknown", "details": "no data", "adx": None}
        ...
        macd_bullish = vals["macd_line"] is not None and vals["macd_signal"] is not None and vals["macd_line"] > vals["macd_signal"]
    ...
    def gather_context(self) -> Dict:
        ...
        ctx = {"timestamp": datetime.now().isoformat(), "date": datetime.now().strftime("%Y-%m-%d")}
        ...
        try:
            ...
            ctx["signals"] = signals
            ctx["risk_exits"] = [{"symbol": e.symbol, "type": e.exit_type, "reason": e.reason} for e in risk_exits]
        ...
        try:
            ctx["risk_state"] = {
                ...
                "entry_prices": {k: round(v, 4) for k, v in self.risk_manager.entry_prices.items()},
            }
        except Exception as e:
        ...
        try:
            ...
            for sig in ctx.get("signals", [])[:3]:  # top 3 signals only
                ...
                if sym_score.get("volatility") is not None:
                    vol_label = "high" if sym_score["volatility"] > 60 else "low" if sym_score["volatility"] < 40 else "moderate"
                similar = journal.find_similar_trades(
                    sym,
                    signal_type=sig.get("signal_type") or sig.get("reason", "").split()[0] if sig.get("reason") else None,
        ...
        try:
            ...
            similar = self.market_intel.find_similar_conditions(current_snapshot, top_k=3)
            ctx["historical_context"] = [
                {"date": s["date"], "content": s["content"][:300], "similarity": round(s["similarity"], 3)}
                for s in similar
            ]
        except Exception as e:
    ...
    def _build_analysis_prompt(self, ctx: Dict) -> str:
        ...
        lines = []
        lines.append("You are Regi, a systematic quantitative trading agent.")
        lines.append("You manage a paper portfolio and learn from every trade outcome.")
        lines.append("Your decisions should be informed by the TRADE JOURNAL data below -- favor signal types")
        lines.append("and regimes where you have historically performed well. Avoid repeating losing patterns.")
        lines.append("")
        lines.append("HARD RULES (enforced by execution engine -- violations are blocked):")
        lines.append(f"- Stop-loss: {STOP_LOSS_PCT:.0%} below entry (automatic, cannot be overridden)")
        lines.append(f"- Circuit breaker: {MAX_PORTFOLIO_DRAWDOWN:.0%} portfolio drawdown (liquidates all)")
        lines.append(f"- Max position size: {MAX_POSITION_PCT:.0%} of portfolio per symbol")
        ...
        lines.append("- allocation_pct: 0.03 to 0.20 (default 0.10). Higher requires high conviction + trend confirmation.")
        lines.append("- hold_hours_override: 6 to 24 (default 12). In strong trend, you may request a shorter hold.")
        lines.append("  If you request <12h, you MUST explain why in your reasoning.")
        lines.append("- sell_pct: 0.25 to 1.0. You decide how much of a position to exit.")
        lines.append("")
        lines.append("CONVICTION RULES:")
        lines.append("- LOW conviction: NEVER executed")
        lines.append("- MEDIUM conviction: requires signal engine confirmation (symbol must appear in ACTIVE SIGNALS)")
        ...
        lines.append(f"## PORTFOLIO (Value: ${portfolio.get('value', 0):.2f}, Cash: ${portfolio.get('cash', 0):.2f})")
        for sym, pos in portfolio.get("positions", {}).items():
            lines.append(f"  {sym}: ${pos['value']:.2f} ({pos['pnl_pct']:+.1f}%) [sector: {pos['sector']}]")
        lines.append("")
        ...
        # Per-symbol trend classification (NEW for P4)
        trends = ctx.get("trends", {})
        if trends:
            lines.append("## TREND REGIME (per symbol)")
            for sym, t in sorted(trends.items()):
                lines.append(f"  {sym}: {t['trend']} ({t['strength']}) -- {t['details']}")
            lines.append("")
            lines.append("  Use these trend classifications to inform your trading decisions:")
            lines.append("  - In UPTREND: favor buying pullbacks, tighten sell triggers, allow larger allocation")
            lines.append("  - In DOWNTREND: favor selling rallies, reduce buy sizes, consider shorter holds")
            lines.append("  - In RANGING: favor mean-reversion (buy low RSI, sell high RSI), use standard sizing")
            lines.append("")

        # Scoring model top/bottom
        scores = ctx.get("scores", {})
        if scores:
            ...
            for sym, sc in sorted_scores[:8]:
                lines.append(f"  {sym}: {sc.get('total_score', 0):.1f} (M:{sc.get('momentum', 0):.0f} T:{sc.get('trend', 0):.0f} V:{sc.get('volatility', 0):.0f} R:{sc.get('risk_adjusted', 0):.0f})")
            lines.append("")
        ...
        if signals:
            ...
            for sig in signals:
                lines.append(f"  {sig.get('symbol', '?')}: {sig.get('action', '?')} (strength {sig.get('strength', 0):.0f}) -- {sig.get('reason', '')[:120]}")
            lines.append("")
        ...
        if risk_exits:
            ...
            for ex in risk_exits:
                lines.append(f"  {ex['symbol']}: {ex['type']} -- {ex['reason'][:100]}")
            lines.append("")
        ...
        if sentiment:
            lines.append("## NEWS SENTIMENT (last 24h)")
            for s in sentiment[:5]:
                lines.append(f"  {s['symbol']}: {s['score']:+.2f} ({s.get('summary', '')[:80]})")
            lines.append("")
        ...
        if risk_events:
            ...
            for event in risk_events[:5]:
                lines.append(f"  [{event.get('timestamp', '?')}] Portfolio: ${event.get('portfolio_value', 0):.2f}")
                for ex in event.get("exits", []):
                    lines.append(f"    {ex['type']} {ex['symbol']}: {ex.get('reason', '')[:100]}")
        ...
        if journal_signals:
            ...
            lines.append("  -> Favor signal types with higher win rates and positive PnL.")
            lines.append("")
        if journal_regimes:
            ...
            lines.append("  -> Adjust position sizing based on regime-specific win rates.")
            lines.append("")
        ...
        if similar_trades:
            ...
            lines.append("  -> Use these precedents to inform your conviction level.")
            lines.append("")
        ...
        if hist:
            ...
            for h in hist:
                lines.append(f"  [{h['date']}] (similarity {h['similarity']:.2f}): {h['content'][:150]}")
            lines.append("")

        # ── Response schema with structured reasoning ─────────────────
        lines.append("## YOUR TASK")
        lines.append("Analyze all data above and provide trade recommendations with STRUCTURED REASONING.")
        lines.append("Every trade must justify itself. Show what indicators support and contradict your thesis.")
        lines.append("")
        lines.append("Respond in this exact JSON format:")
        lines.append('```json')
        lines.append('{')
        lines.append('  "market_assessment": "1-2 sentence summary of current conditions",')
        ...
        lines.append('        "thesis": "1-2 sentence core thesis for this trade",')
        lines.append('        "trend_regime": "uptrend|downtrend|ranging -- what you see for this symbol",')
        lines.append('        "supporting": ["list of indicators/signals that support this trade"],')
        lines.append('        "contradicting": ["list of indicators/signals that argue against this trade"],')
        lines.append('        "regime_consideration": "how the current VIX/macro regime affects this trade",')
        lines.append('        "similar_past_outcome": "what happened in similar past trades (or null if none)",')
        lines.append('        "risk_note": "what could go wrong and how risk is managed"')
        lines.append('      }')
        lines.append('    }')
        lines.append('  ],')
        lines.append('  "rebalance_recommended": false,')
        lines.append('  "hold_rationale": "if no trades, explain why holding is the right call",')
        lines.append('  "watchlist": ["symbols to monitor closely and why"]')
        lines.append('}')
        lines.append('```')
        lines.append("")
        lines.append("IMPORTANT: The 'reasoning' object is REQUIRED for every trade. Trades without it are rejected.")

        return "\n".join(lines)

    def _call_llm(self, prompt: str) -> Optional[str]:
        """Call LLM for trading analysis. Try DeepSeek first, fall back to OpenAI."""
        # Try OpenAI (more reliable for JSON output)
        if OPENAI_API_KEY:
            try:
                import openai
    ...
    def _build_decision_context(self, trade: Dict, ctx: Dict, guardrail_notes: List[str] = None) -> str:
        ...
        context = {
            ...
            "reasoning": reasoning,
            "trend_regime": reasoning.get("trend_regime") or ctx.get("trends", {}).get(trade.get("symbol"), {}).get("trend"),
    ...
    def execute_trades(self, analysis: Dict, ctx: Dict) -> List[Dict]:
        ...
        for exit_sig in ctx.get("risk_exits", []):
            ...
            if sym in self.portfolio.positions:
                ...
                executed.append({
                    "symbol": sym, "side": "SELL", "amount": "all",
                    "reason": exit_sig["reason"], "type": "risk_exit",
                })
        ...
        for trade in trades:
            if llm_trade_count >= MAX_LLM_TRADES:
                logger.info(f"LLM trade limit reached ({MAX_LLM_TRADES}) -- skipping remaining")
                break
            ...
            if conviction == "medium" and sym not in signal_symbols:
                logger.info(f"LLM skip: medium-conviction {action} {sym} (no signal confirmation)")
                continue
            ...
            if conviction == "high" and trend_label in ("uptrend", "downtrend") and trend_strength in ("strong", "moderate"):
                alloc_cap = MAX_ALLOC_PCT
                guardrail_notes.append(f"alloc_cap_raised_to_{MAX_ALLOC_PCT:.0%}_trend_confirmed")

            amount_pct = max(MIN_ALLOC_PCT, min(requested_pct, alloc_cap))
            if amount_pct != requested_pct:
                guardrail_notes.append(f"allocation_clamped_{requested_pct:.2f}_to_{amount_pct:.2f}")
            ...
            self.portfolio.set_trade_context(
                sym, signal_type="llm", signal_strength=None,
                exit_reason="llm_signal", market_regime=regime_label,
            )
            ...
            if action == "BUY":
                ...
                if sym in self.portfolio.positions:
                    price = self.portfolio.get_current_price(sym)
                    current_pos_value = price * self.portfolio.positions[sym]["quantity"]
                new_pct = (current_pos_value + amount_usd) / portfolio_value
                if new_pct > MAX_POSITION_PCT:
                    logger.warning(f"LLM skip: {sym} buy would exceed {MAX_POSITION_PCT:.0%} position limit")
                    continue
                ...
                if amount_usd > available_cash:
                    guardrail_notes.append(f"amount_reduced_cash_reserve_{amount_usd:.2f}_to_{available_cash:.2f}")
                    amount_usd = available_cash
                if amount_usd < 10:
                    logger.info(f"LLM skip: {sym} buy -- insufficient cash after reserve")
                    continue

                success = self.portfolio.execute_buy(sym, amount_usd)
                if success:
                    self.risk_manager.record_entry(sym, self.portfolio.get_current_price(sym))
                    executed.append({
                        "symbol": sym, "side": "BUY", "amount": round(amount_usd, 2),
                ...
                if entry_date_str:
                    try:
                        ...
                        if held_hours < hold_hours:
                            logger.info(f"LLM skip: {sym} sell -- held {held_hours:.1f}h < {hold_hours:.0f}h minimum")
                            continue
                    except Exception:
                        pass
                ...
                if sell_pct is not None:
                    ...
                    if sell_frac != sell_pct:
                        guardrail_notes.append(f"sell_pct_clamped_{sell_pct}_to_{sell_frac}")
                else:
                ...
                if success:
                    if sym not in self.portfolio.positions:
                        self.risk_manager.remove_position(sym)
                    executed.append({
                        "symbol": sym, "side": "SELL", "amount": round(sell_qty, 6),
                        "conviction": conviction,
                        "reasoning": reasoning,
                        "guardrail_adjustments": guardrail_notes,
                    })
    ...
    def run_daily_review(self) -> Dict:
        """
        Run the full daily trading review:
        1. Gather all context
        2. Get LLM analysis
        3. Execute recommended trades
        4. Handle smart rebalancing
        5. Store analysis in market intelligence
        ...
        logger.info(f"Context gathered: {len(ctx.get('signals', []))} signals, regime={ctx.get('regime', {}).get('label', '?')}")
        ...
        if not analysis:
            logger.warning("LLM analysis failed — falling back to rule-based signals only")
            analysis = {
        ...
        if analysis.get("rebalance_recommended", False):
            try:
                scores = {sym: data.get("total_score", 50) for sym, data in ctx.get("scores", {}).items()}
                rebalance_result = self.smart_rebalancer.execute_rebalance(self.portfolio, scores)
        ...
        try:
            ...
            for trade in executed:
                ...
                if isinstance(reasoning, dict) and reasoning.get("thesis"):
                    ...
                    if supporting:
                        rationale_text += f" Supporting: {', '.join(str(s) for s in supporting[:5])}."
                ...
                self.market_intel.store_trade_rationale(
                    trade["symbol"], trade["side"], rationale_text, ctx["date"],
                )
        except Exception as e:
    ...
    def format_telegram_report(self, result: Dict) -> str:
        ...
        title = "**Morning Strategic Review**" if mode == "morning" else "**Midday Tactical Check**"
        lines.append(title)
        ...
        regime_emoji = {"risk-on": "🟢", "neutral": "🟡", "risk-off": "🔴"}.get(regime.get("label", ""), "❓")
        lines.append(f"**Regime:** {regime_emoji} {regime.get('label', '?')} (VIX {regime.get('vix', '?')})")
        lines.append("")

        # Portfolio
        portfolio = ctx.get("portfolio", {})
        lines.append(f"**Portfolio:** ${portfolio.get('value', 0):.2f} (Cash: ${portfolio.get('cash', 0):.2f})")
        lines.append("")
        ...
        if executed:
            ...
            for t in executed:
                ...
                if isinstance(amount, (int, float)):
                    amount = f"${amount:.2f}"
                lines.append(f"{emoji} {t['side']} {t['symbol']} ({amount}) [{t.get('conviction', '?')}]")
                ...
                if isinstance(reasoning, dict) and reasoning.get("thesis"):
                    ...
                    supporting = reasoning.get("supporting", [])
                    if supporting:
                        lines.append(f"   +: {', '.join(str(s) for s in supporting[:3])}")
                    contradicting = reasoning.get("contradicting", [])
                    if contradicting:
                        lines.append(f"   -: {', '.join(str(s) for s in contradicting[:3])}")
                elif t.get("reason"):
        ...
        if rebalance and rebalance.get("rebalanced"):
            lines.append(f"**Rebalanced:** {rebalance['trades_completed']} trades (fees ${rebalance.get('total_fees', 0):.4f})")
            lines.append("")
    ...
    def run_midday_review(self) -> Dict:
        ...
        strong_signals = [s for s in ctx.get("signals", []) if s.get("strength", 0) >= 70]
        ctx["signals"] = strong_signals

        if not strong_signals and not ctx.get("risk_exits") and not ctx.get("recent_risk_events"):
            logger.info("Midday: no strong signals or risk events — skipping LLM")
            return {
                "timestamp": ctx["timestamp"],
                "context": ctx,
                "analysis": {"market_assessment": "Quiet midday — no strong signals", "trades": [], "risk_level": "low"},
                "executed_trades": [],
                "rebalance": None,
                "mode": "midday",
            }
        ...
        if not analysis:
            analysis = {"market_assessment": "LLM unavailable", "trades": [], "risk_level": "unknown"}

        # Only execute high-conviction trades
        if analysis.get("trades"):
            analysis["trades"] = [t for t in analysis["trades"] if t.get("conviction") == "high"]
    ...
    def _build_midday_prompt(self, ctx: Dict) -> str:
        ...
        lines = []
        lines.append("You are Aleph, a quantitative trading agent. This is a MIDDAY tactical check.")
        ...
        if positions:
            pos_str = ", ".join(f"{s}: ${p['value']:.0f} ({p['pnl_pct']:+.1f}%)" for s, p in positions.items())
            lines.append(f"Positions: {pos_str}")
        lines.append("")

        # Regime (compact)
        regime = ctx.get("regime", {})
        lines.append(f"Regime: {regime.get('label', '?')} (VIX {regime.get('vix', '?')}) | BTC-SPY: {regime.get('btc_spy_corr', '?')}")
        lines.append("")
        ...
        if signals:
            ...
            for sig in signals:
                lines.append(f"  {sig.get('symbol')}: {sig.get('action')} strength={sig.get('strength', 0):.0f} | {sig.get('reason', '')[:80]}")
            lines.append("")
        ...
        if risk_events:
            ...
            for event in risk_events[:3]:
                for ex in event.get("exits", []):
                    lines.append(f"  {ex['type']} {ex['symbol']}: {ex.get('reason', '')[:80]}")
            lines.append("")

        lines.append("Respond in JSON: {\"market_assessment\": \"...\", \"risk_level\": \"...\", \"trades\": [{\"symbol\": \"...\", \"action\": \"BUY|SELL\", \"amount_pct\": 0.05, \"conviction\": \"high\", \"rationale\": \"...\"}], \"watchlist\": [\"...\"]}")
        ...
        if OPENAI_API_KEY:
            try:
                ...
                resp = client.chat.completions.create(
                    model="gpt-4o-mini",
                    messages=[
                        {"role": "system", "content": "You are a quantitative trading agent. Respond with valid JSON only."},
                        {"role": "user", "content": prompt},
                    ],
                    temperature=0.1,
                    max_tokens=800,
...
def main():
    ...
    parser.add_argument("--run", action="store_true", help="Run morning strategic review")
    parser.add_argument("--midday", action="store_true", help="Run midday tactical review")
    parser.add_argument("--no-telegram", action="store_true", help="Skip Telegram report")
    parser.add_argument("--context-only", action="store_true", help="Just gather and print context")
