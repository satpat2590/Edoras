...
class AutomatedPortfolioReporter:
    ...
    def generate_report(self) -> str:
        ...
        if len(portfolio) > 0:
            ...
            if top_asset_pct > 50:
                report_lines.append("")
                report_lines.append(f"⚠️ {portfolio[0]['currency']} is {top_asset_pct:.1f}% of portfolio")
    ...
    def send_via_openclaw(self, report_text: str) -> bool:
        ...
        try:
            ...
            import subprocess
            cmd = ["openclaw", "message", "send", "--target", self.telegram_target, "--message", report_text]
            
            # Set timeout and run
            result = subprocess.run(cmd, capture_output=True, text=True, timeout=30)
            
            if result.returncode == 0:
                print(f"✅ Report sent to Telegram at {datetime.now().strftime('%H:%M:%S')}")
                return True
            else:
                print(f"❌ Error sending to Telegram: {result.stderr[:100]}")
                return False
                
        except subprocess.TimeoutExpired:
            print("❌ Timeout sending to Telegram")
            return False
        except Exception as e:
    ...
    def run_and_send(self) -> bool:
        ...
        print(f"Automated Portfolio Report - {self.report_date.strftime('%Y-%m-%d %H:%M')}")
        print("-" * 50)
        ...
        try:
            ...
            # Send via Telegram
            success = self.send_via_openclaw(report)
    ...
    def log_execution(self, success: bool, error_msg: str = ""):
        ...
        log_file = "automated_reports.log"
        log_entry = {
