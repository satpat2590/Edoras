...
class CorrelationTracker:
    ...
    def rolling_correlation(
        ...
        if len(combined) < window:
            logger.warning(f"Insufficient overlap for {symbol_a}/{symbol_b}: {len(combined)} days")
            return pd.Series(dtype=float)

        return combined["a"].rolling(window).corr(combined["b"]).dropna()

    def current_correlation(self, symbol_a: str, symbol_b: str, window: int = 30) -> Optional[float]:
    ...
    def portfolio_beta_vs_btc(self, portfolio_symbols: List[str] = None, window: int = 60) -> Optional[float]:
        """
        ...
        combined = pd.DataFrame({"port": port_returns, "btc": btc_returns}).dropna().tail(window)
    ...
    def save_daily_snapshot(self):
        ...
        for window in [30, 60, 90]:
            for sym_a, sym_b in [("BTC-USD", "SPY"), ("BTC-USD", "QQQ"), ("BTC-USD", "ETH-USD")]:
        ...
        conn.commit()
        conn.close()
        logger.info(f"Saved correlation snapshot for {today}: regime={regime}, VIX={vix}")
...
def main():
    ...
    parser.add_argument("--matrix", action="store_true", help="Print correlation matrix")
