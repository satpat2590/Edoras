...
def get_portfolio_state():
    ...
    state_file = os.path.join(
        ...
        "..",
        "state/paper_portfolio_full_state.json",
    )
    try:
...
def trace_symbol(
    ...
    if held:
        ...
        print(
            f"  Position:  HELD (qty={qty:.6g}, value=${pos_value:.2f}, "
    ...
    print(
        f"  Limits:    max_pos={max_pct:.0%}, min_trade=${min_trade:.0f}, min_hold={min_hold}h"
    )
    ...
    print(
        f"  Cash:      ${capital:.2f} available (${available_cash:.2f} after 5% reserve)"
    )
    ...
    if held:
        pos_value = positions[symbol]["quantity"] * (
            current_price if "current_price" in dir() else 0
        )
        pos_pct = pos_value / portfolio_value if portfolio_value > 0 else 0
        if pos_pct >= max_pct * 0.9:
            print(
                f"    ❌ position_held: alloc {pos_pct:.1%} ≥ {max_pct * 0.9:.1%} (90% of max)"
            )
    ...
    if buy_50 < min_trade:
        print(
            f"    ❌ insufficient_cash: str=50 → ${buy_50:.2f} < min ${min_trade:.0f}"
        )
        blocked = True
    else:
        print(
            f"    ✅ Cash OK: str=50 → ${buy_50:.2f}, str=80 → ${min(portfolio_value * 0.10, available_cash):.2f}"
        )
        ...
        if entry_date_str:
            try:
                ...
                if held_hours < min_hold:
                    print(
                        f"    ❌ min_hold_period: held {held_hours:.1f}h < {min_hold}h"
                    )
                else:
    ...
    if rows:
        for r in rows:
            ...
            print(
                f"    {r[4]:20s} {r[0]:4s} str={r[1]:5.1f} {status}{skip}  {r[3][:16]}"
            )
    else:
        print(f"    (none)")
...
def show_recent_skips(conn: sqlite3.Connection, limit: int = 30):
    ...
    print(
        f"  {'Strategy':<20s} {'Symbol':<12s} {'Act':>4s} {'Str':>5s} {'Skip Reason':<30s} {'Time':<16s}"
    )
    print(f"  {'-' * 20} {'-' * 12} {'-' * 4} {'-' * 5} {'-' * 30} {'-' * 16}")
    for r in rows:
        print(
            f"  {r[0]:<20s} {r[1]:<12s} {r[2]:>4s} {r[3]:5.1f} {r[4]:<30s} {r[5][:16]}"
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Verify signal flow through execution pipeline"
    )
    parser.add_argument("symbol", nargs="?", help="Trace a specific symbol")
    parser.add_argument(
        "--recent", action="store_true", help="Show recent skip reasons"
    )
    ...
    print(
        f"  Portfolio: ${portfolio_value:.2f} | Cash: ${capital:.2f} | "
        f"Positions: {len(positions)}"
    )
    print(
        f"  Symbols: {len(symbols)} routed | "
    ...
    risk_state_file = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "..", "state/risk_state.json"
    )
    try:
