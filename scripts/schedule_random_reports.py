...
class ReportScheduler:
    ...
    def load_config(self) -> dict:
        ...
        default_config = {
            ...
            "script_path": os.path.join(
                os.path.dirname(__file__), "run_portfolio_report.sh"
            ),
            "log_file": os.path.join(
                os.path.dirname(__file__), "logs/report_schedule.log"
            ),
        }
    ...
    def generate_random_times(self) -> list:
        ...
        num_reports = random.randint(
            config["min_reports_per_day"], config["max_reports_per_day"]
        )
        ...
        for i in range(num_reports):
            ...
            for attempt in range(max_attempts):
                ...
                candidate_time = self.today.replace(
                ...
                for existing_time in times:
                    time_diff = abs(
                        (candidate_time - existing_time).total_seconds() / 3600
                    )
                ...
                if valid:
                    ...
                    print(
                        f"Warning: Could not find well-spaced time for report {i + 1}, using any time"
                    )
                    times.append(candidate_time)
    ...
    def schedule_with_at(self, report_time: datetime) -> bool:
        ...
        try:
            result = subprocess.run(
                at_command, shell=True, capture_output=True, text=True
            )
    ...
    def schedule_daily_reports(self):
        ...
        if successes > 0:
            ...
            with open(cron_file, "w") as f:
                f.write(
                    f"# Today's random report schedule - {self.today.strftime('%Y-%m-%d')}\n"
                )
                for t in times:
                    f.write(
                        f"{t.strftime('%H %M * * *')} {self.config['script_path']}\n"
                    )
