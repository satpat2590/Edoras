...
SCRIPT="$WORKSPACE/daily_portfolio_report.py"
...
if [ $EXIT_CODE -eq 0 ]; then
    ...
    openclaw message send --target "$TARGET_CHAT" --message "⚠️ Daily crypto portfolio report failed. Check logs." 2>/dev/null || true
fi
