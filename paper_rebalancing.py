...
from config import MAX_SECTOR_PCT, MAX_POSITION_PCT, get_sector
...
def send_telegram_message(message: str, chat_id: str = None):
    ...
    try:
        ...
        cmd = f"openclaw message send --target {chat_id} --message '{escaped_message}'"
        os.system(cmd)
        return True
...
def main():
    ...
    try:
        ...
        context = json.dumps({
            ...
            "top_5_symbols": top_5,
            "scores": {
                sym: float(scores_df[scores_df['symbol'] == sym]['total_score'].values[0])
                for sym in top_5
                if len(scores_df[scores_df['symbol'] == sym]['total_score'].values) > 0
            },
        ...
        if rebalancing_cost > 0:
            report_lines.append(f"• Rebalancing cost: ${rebalancing_cost:.2f} (transaction fees)")
        else:
        ...
        for i, symbol in enumerate(top_5, 1):
            ...
            symbol_score = scores_df[scores_df['symbol'] == symbol]['total_score'].values
        ...
        if telegram_success:
            ...
            print("⚠️ Could not send to Telegram (OpenClaw CLI may not be available)")
