...
# Log file
LOG_FILE="run_portfolio_report.log"
...
if ! command -v openclaw &> /dev/null; then
    echo "ERROR: openclaw command not found in PATH" >> "$LOG_FILE"
    echo "ERROR: openclaw command not found in PATH"
    echo "PATH: $PATH" >> "$LOG_FILE"
    exit 1
fi

echo "OpenClaw CLI version: $(openclaw --version 2>&1)" >> "$LOG_FILE"

# Run the automated report
echo "Running automated portfolio report..." >> "$LOG_FILE"
python3 automated_portfolio_report.py >> "$LOG_FILE" 2>&1
EXIT_CODE=$?
